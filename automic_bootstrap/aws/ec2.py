...
def ensure_security_group(ec2, vpc_id, sg_name):
    ...
    sg = ec2.create_security_group(GroupName=sg_name, Description='Automic Security Group', VpcId=vpc_id)
    sg_id = sg['GroupId']
    ...
    for port in ports:
        while True:
            try:
                ...
                if 'InvalidPermission.Duplicate' in str(e): break
                if 'Throttling' in str(e): time.sleep(2); continue
                raise
    return sg_id
...
def ensure_key_pair(name: str, key_path: Path, ec2):
    ...
    try:
        ec2.describe_key_pairs(KeyNames=[name]); ec2.delete_key_pair(KeyName=name)
    except botocore.exceptions.ClientError:
        pass
    _delete_key_file_safely(key_path)
    kp = ec2.create_key_pair(KeyName=name)
    key_path.write_text(kp['KeyMaterial']); key_path.chmod(0o600)
    return str(key_path)
...
def wait_for_instances(ec2, instance_ids):
    ec2.get_waiter('instance_running').wait(InstanceIds=instance_ids, WaiterConfig={'Delay': 15, 'MaxAttempts': 40})
    ec2.get_waiter('instance_status_ok').wait(InstanceIds=instance_ids, WaiterConfig={'Delay': 15, 'MaxAttempts': 40})
